...
int AxisMatrixMult(u32 UartBaseAddress);
...
//            xil_printf("Row %d Total Sum Before /256: %d\n", i, matrixSol[i][j]);
            matrixSol[i][j] = matrixSol[i][j] >> 8 ;
        }
    }
}
...
int main(void)
{
	 ...
	 */
	Status = AxisMatrixMult(UART_BASEADDR);
